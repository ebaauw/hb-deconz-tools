// hb-deconz-tools/lib/ApiClient.js
//
...
import { ApiResponse } from 'hb-deconz-tools/ApiResponse'
...
class ApiClient extends HttpClient {
  ...
  async get (resource) {
    ...
    switch (path[0]) {
      ...
      case 'groups':
        ...
        // falls through
      case 'devices':
        if (['ddf', 'ddffull', 'introspect'].includes(path[path.length - 1])) {
          path = []
          break
        }
        // falls through
  ...
  /** Issue a PUT request to `/api/`_API_key_`/`_resource_.
    ...
    * @return {ApiResponse} response - The response.
    * @throws {ApiError} In case of error, except for non-critical API errors.
    */
  ...
  /** Obtain an API key and set {@link ApiClient#apiKey apiKey}.
    *
    * Calls {@link ApiClient#post post()} to issue a POST request to `/api`.
    *
    ...
    * @throws {ApiError} In case of API error.
    */
  ...
  /** Delete the API key and clear {@link ApiClient#apiKey apiKey}.
    * @throws {HttpError} In case of HTTP error.
    * @throws {ApiError} In case of API error.
    */
  async deleteApiKey () {
  ...
  /** Unlock the gateway to allow creating a new API key.
    *
    * Calls {@link ApiClient#put put()} to issue a PUT request to
  ...
  /** Restart the gateway.
    *
    * Calls {@link ApiClient#post post()} to issue a POST request to
  ...
  checkServerIdentity (hostname, cert) {
    // Check whether certificate is signed by CA.
    let c = cert
    while (c.issuerCertificate != null && c.issuerCertificate !== c) {
      c = c.issuerCertificate
    }
    if (c.ca) {
      delete this._options.fingerprint
      return
    }

    // Check whether self-signed certificate matches pinned certificate.
    if (this._options.fingerprint != null && this._options.fingerprint !== '') {
      if (cert.fingerprint512 !== this._options.fingerprint) {
        return new Error('SSL certificate fingerprint mismatch')
      }
      return
    }

    // TODO check subject for bridgeid

    // Pin new self-signed certificate.
    this._options.fingerprint = cert.fingerprint512
  }

  /** Issue an HTTP request to the deCONZ gateway.
    *
    * This method does the heavy lifting for {@link AplClient#get get()},
    * {@link ApiClient#put put()}, {@link ApiClient#post post()},
    * and {@link ApiClient#delete delete()}.
  ...
  async request (method, resource, body = null, retry = 0) {
    try {
      ...
      const response = new ApiResponse(httpResponse)
      if (method === 'GET' && resource === '/config' && response.body.whitelist == null) {
        // unauthorised GET /config
        const e = { address: resource, description: 'unauthorized user', type: 1 }
        response.errors.push(new ApiError(e, response))
      }
      for (const error of response.errors) {
        /** Emitted for each API error returned by the or deCONZ gateway.
          *
          * @event ApiClient#error
          * @param {HttpClient.HttpError|ApiError} error - The error.
          */
