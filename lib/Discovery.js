...
import { HttpClient, HttpError } from 'hb-lib-tools/HttpClient'
...
class Discovery {
  ...
  async config (host, https = false) {
    ...
    try {
      const { jsonBody: body } = await client.get('/config')
      if (
  ...
  async description (host, https = false) {
    ...
    const client = new HttpClient(options)
    const { xmlBody } = await client.get('/description.xml')
    return xmlBody
  }
    ...
    try {
      ...
      if (Array.isArray(body)) {
        for (const gateway of body) {
          ...
          this.#found(name, gateway.id.toUpperCase(), host)
        }
      }
    } catch (error) {
