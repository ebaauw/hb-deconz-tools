...
class Discovery extends EventEmitter {
  ...
  #checkSsl (host, options) {
    const { port } = OptionParser.toHost('host', host)
    if (port === 443) {
      options.https = true
      options.selfSignedCertificate = true
      options.checkServerIdentity = (hostname, cert) => {
        if (cert.issuerCertificate?.ca) {
          return
        }
        this.fingerprint = cert.fingerprint512
      }
    }
  }
