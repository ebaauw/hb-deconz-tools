...
class DeconzTool extends CommandLineTool {
  ...
  async _main () {
    ...
    try {
      ...
      this.gatewayDescription = await this.deconzDiscovery.description(
        this.clargs.options.host
      )
    } catch (error) {
      if (error.request == null) {
    ...
    if (this.clargs.command === 'description') {
      return this[this.clargs.command](this.clargs.args)
    }
    ...
    if (this.gatewayDescription?.root?.device?.['deconz:info']?.httpsPort != null) {
      this.clargs.options.https = true
      this.clargs.options.host = this.clargs.options.host.split(':')[0] + ':' +
        this.gatewayDescription.root.device['deconz:info'].httpsPort
      if (this.gateways[this.bridgeid]?.fingerprint != null) {
  ...
  async eventlog (...args) {
    ...
    options.wss = this.gatewayConfig.https && websocketport_wss != null
  ...
  async description (...args) {
    ...
    parser
      ...
      .parse(...args)
    const jsonFormatter = new JsonFormatter(options)
    const json = jsonFormatter.stringify(this.gatewayDescription)
    this.print(json)
  }
