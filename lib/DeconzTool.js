...
import { CommandLineTool, CommandLineParser, UsageError, b, u } from 'hb-lib-tools/CommandLineTool'
import { JsonFormatter } from 'hb-lib-tools/JsonFormatter'
import { toHost, toInt, toPath, toString } from 'hb-lib-tools/OptionParser'
...
const usage = {
  ...
  otau: `${b('otau')} [${b('-hv')}]`,
  stale: `${b('stale')} [${b('-hv')}] [${b('-m')} ${u('minutes')}]`,
...
const description = {
  ...
  stale: 'List Zigbee devices that have not been seen for a while.',
...
Parameters:
  ...
  ${b('-v')}, ${b('--verbose')}
  Print full API output.`,
  stale: `${description.deconz}

Usage: ${b('deconz')} ${usage.stale}
...
Parameters:
  ...
  ${b('-m')}, ${b('--minutes')} ${u('minutes')}
  List devices that have not been seen for ${u('minutes')} minutes (default: 60).`,
...
class DeconzTool extends CommandLineTool {
  constructor (pkgJson) {
    ...
    this._packageJson = pkgJson ?? defaultPackageJson
    this.usage = usage.deconz
    this.fileName = path.join(process.env.HOME, '.deconz')
    try {
      this.readGateways()
    } catch (error) {
  ...
  readGateways () {
    ...
    try {
      ...
      this.warn('%s: file corrupted', this.fileName)
      this.gateways = {}
    }
  }
  ...
  writeGateways () {
    ...
    writeFileSync(this.fileName, text, { mode: 0o600 })
  }
  ...
  parseArguments () {
    ...
    parser
      .helpFlag('h', 'help', help.deconz)
      .versionFlag('V', 'version')
      .debugFlag('D', 'debug')
      .option('H', 'host', (value) => {
        const { port } = toHost(value, { key: 'host', userInput: true })
        clargs.options.host = value
        clargs.options.https = port === 443
      })
      .option('K', 'apiKey', (value) => {
        clargs.options.apiKey = toString(
  ...
  async _main () {
    ...
    try {
      ...
      if (error.request == null) {
        this.fatal('%s: %s', this.clargs.options.host, error)
        return
      }
      this.fatal('%s: deCONZ gateway not found', this.clargs.options.host)
      return
    }

    this.clargs.options.logger = this
    this.clargs.options.https = this.gatewayConfig.https
    ...
    parser
      ...
      .remaining((list) => {
        ...
        clargs.resource = list.length === 0
          ? '/'
          : toPath(list[0], { key: 'resource', userInput: true })
      })
      .parse(...args)
  ...
  async resourceCommand (command, ...args) {
    ...
    parser
      .helpFlag('h', 'help', help[command])
      .flag('v', 'verbose', () => { clargs.options.verbose = true })
      .parameter('resource', (resource) => {
        clargs.resource = toPath(resource, { key: 'resource', userInput: true })
        if (clargs.resource === '/') {
  ...
  async description (...args) {
    ...
    const options = {}
    parser
      .helpFlag('h', 'help', help.description)
      .flag('s', 'sortKeys', () => { options.sortKeys = true })
      .parse(...args)
    const jsonFormatter = new JsonFormatter(options)
  ...
  async getApiKey (...args) {
    ...
    parser
      .helpFlag('h', 'help', help.getApiKey)
      .parse(...args)
  ...
  async #addOtau (map, otau, rtype, id) {
    ...
    if (map[fileNamePrefix] != null) {
      map[fileNamePrefix].resources.push('/' + rtype + '/' + id)
      return
    }
    map[fileNamePrefix] = {
      fileNamePrefix,
    ...
    return this.firmwareClient.check(fw)
  }

  async otau (...args) {
    const parser = new CommandLineParser(this)
    parser
      .helpFlag('h', 'help', help.otau)
      .parse(...args)

    const { FirmwareClient } = await import('hb-deconz-tools/FirmwareClient')
    this.firmwareClient = new FirmwareClient({ logger: this })

    const map = {}
    const lights = await this.client.get('/lights')
    for (const id in lights) {
      try {
        await this.#addOtau(map, lights[id]?.capabilities?.otau, 'lights', id)
      } catch (error) { this.warn(error) }
    }
    const sensors = await this.client.get('/sensors')
    for (const id in sensors) {
      try {
        await this.#addOtau(map, sensors[id].capabilities?.otau, 'sensors', id)
      } catch (error) { this.warn(error) }
    }
    const jsonFormatter = new JsonFormatter(
  ...
  async stale (...args) {
    ...
    const parser = new CommandLineParser(this)
    parser
      .helpFlag('h', 'help', help.stale)
      .option('m', 'minutes', (value, key) => {
        minutes = toInt(value, { key, min: 1, max: 24 * 60, userInput: true })
      })
      .parse(...args)
    ...
    for (const id in lights) {
      n++
      if (new Date(lights[id].lastseen) < threshold) {
        nStale++
        stale['/lights/' + id] = lights[id].lastseen
      }
    }
    this.debug('%d/%d stale lights', nStale, n)
    const sensors = await this.client.get('/sensors')
    n = 0
    nStale = 0
    for (const id in sensors) {
      if (!sensors[id].type.startsWith('ZHA')) {
        continue
      }
      n++
      if (new Date(sensors[id].lastseen) < threshold) {
        nStale++
        stale['/sensors/' + id] = sensors[id].lastseen
      }
    }
    this.debug('%d/%d stale zigbee sensors', nStale, n)
    this.jsonFormatter = new JsonFormatter()
    const json = this.jsonFormatter.stringify(stale)
    this.print(json)
  }
  ...
  async probe (...args) {
    ...
    parser
      .helpFlag('h', 'help', help.probe)
      .flag('v', 'verbose', () => { clargs.verbose = true })
      .option('t', 'timeout', (value) => {
        toInt(value, { key: 'timeout', min: 1, max: 10, userInput: true })
