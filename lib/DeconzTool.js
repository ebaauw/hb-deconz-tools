...
Parameters:
  ...
  ${u('resource')}
  Resource of device to check for new firmware.`,
...
class DeconzTool extends CommandLineTool {
  ...
  async addOtau (map, otau) {
    if (otau?.manufacturercode == null || otau?.imagetype == null || otau?.fileversion == null) {
      return
    }
    if (otau.manufacturercode === 0 || otau.imagetype === 0 || otau.fileversion === 0) {
      return
    }
    ...
    map[fileNamePrefix] = {
      manufacturerCode: otau.manufacturercode,
      imageType: otau.imagetype,
      fileVersion: otau.fileversion
    }
    // if (otau.manufacturercode === 0x100B) {
    //   await this.checkHueFirmware(map[fileNamePrefix])
    // }
  }
  ...
  async otau (...args) {
    ...
    for (const id in lights) {
      try {
        await this.addOtau(map, lights[id]?.otau)
      } catch (error) { this.warn(error) }
    }
    const sensors = await this.client.get('/sensors')
    for (const id in sensors) {
      try {
        await this.addOtau(map, sensors[id]?.otau)
      } catch (error) { this.warn(error) }
    }
    const jsonFormatter = new JsonFormatter(
