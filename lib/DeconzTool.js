...
class DeconzTool extends CommandLineTool {
  ...
  async _main () {
    ...
    this.deconzDiscovery
      .on('error', (error) => {
        this.warn('discovery error: %s', error)
      })
  ...
  async checkHueFirmware (otau) {
    if (this.hueFirmwareClient == null) {
      this.hueFirmwareClient = new HttpClient({
  ...
  async checkIkeaFirmware (otau) {
    if (this.ikeaFirmwareClient == null) {
      this.ikeaFirmwareClient = new HttpClient({
      ...
      for (const image of body) {
        if (image.fw_type === 2 && image.fw_image_type != null) {
          ...
          this.ikeaMap[image.fw_image_type] = {
      ...
      this.debug(
        '%s: found %d firmware files', this.ikeaFirmwareClient.name,
        Object.keys(this.ikeaMap).length
      )
    }
    const update = this.ikeaMap[otau.imageType]
    if (update == null) {
      return false
    }
    otau.url = update.url
  ...
  async checkFirmware (otau) {
    if (this.koenkkClient == null) {
      this.koenkkClient = new HttpClient({
        host: 'raw.githubusercontent.com',
        https: true,
        logger: this,
        path: '/Koenkk/zigbee-OTA/master'
      })
      const { body } = await this.koenkkClient.get('/index.json')
      this.koenkkMap = {}
      ...
      for (const image of images) {
        ...
        if (this.koenkkMap[fileNamePrefix] != null) {
          this.koenkkMap[fileNamePrefix].duplicate = true
          continue
        }
        this.koenkkMap[fileNamePrefix] = {
          url: image.url,
          sha512: image.sha512
        }
      }
      this.debug(
        '%s: found %d firmware files', this.koenkkClient.name,
        Object.keys(this.koenkkMap).length
      )
    }
    const update = this.koenkkMap[otau.fileNamePrefix]
    if (update == null || update.duplicate) {
      return false
    }
    otau.url = update.url
    otau.sha256 = update.sha256
