// hb-deconz-tools/lib/WsClient.js
//
...
class WsClient extends EventEmitter {
  /** Instantiate a new web socket client.
    ...
    * @param {boolean} [params.wss=false] - Use secure web socket connection.
    ...
    * them.<br>
    * When specified, {@link WsClient#event:notification notification}
    * events are emitted, in lieu of {@link WsClient#event:changed changed},
    * {@link WsClient#event:added added}, and
    * {@link WsClient#event:sceneRecall sceneRecall} events.
    */
  constructor (params = {}) {
    super()
    this.config = {
    ...
    for (const f of ['warn', 'log', 'debug', 'vdebug', 'vvdebug']) {
      this[f] = this.config.logger?.[f]?.bind(this.config.logger) ?? (() => {})
    }
  }
  ...
  listen () {
    this.reconnect = true
    const protocol = this.config.wss ? 'wss' : 'ws'
    const url = protocol + '://' + this.config.hostname + ':' + this.config.port
    const options = { family: 4 }
    if (this.config.wss && this.config.fingerprint != null) {
    ...
    this.ws
      ...
      .on('upgrade', (response) => {
        ...
        if (cert.fingerprint512 !== this.config.fingerprint) {
          this._error = true
          this.warn('SSL certificate fingerprint mismatch')
        }
      })
      .on('message', (data, flags) => {
        try {
          ...
          /** Emitted when an unknown notification has been received, or when
            ...
            * {@link WsClient constructor}.
            * @event WsClient#notification
      ...
      .on('close', async () => {
        ...
        this.debug('connection to %s closed', url)
        if (retryTime > 0) {
